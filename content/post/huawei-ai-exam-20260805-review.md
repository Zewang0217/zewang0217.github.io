...
先说清楚一个诚实声明：转贴原文时，不少选择题的**具体数值在公式里丢失了**（LaTeX 没有跟着复制过来）。所以本文对这类题只给方法和通式，不给编造的数值——这比假装有完整数据更有用。
...
两阶段模拟：第一遍 $O(n^2)$ 统计每个点的邻居数、标记核心点；第二遍非核心点去查核心点集合，距离 $\le \varepsilon$ 就是边界点。总复杂度 $O(n^2)$，在 $n \le 1000$ 量级（原题数据范围粘贴时部分丢失，按 $O(n^2)$ 可过）完全没问题。
...
> 当输入较大时直接算 $e^{x_i}$ 会溢出，正确的稳定化实现是：**对所有 $x_i$ 减去最大值** $x_i - \max_j x_j$ 再算指数。
...
**#4 欧氏距离（✅）**：两个嵌入向量 $u, v$ 的欧氏距离是 $\|u - v\|_2 = \sqrt{\sum_i (u_i - v_i)^2}$，距离越小越相似。（原题数值丢失，方法不变。）

**#8 余弦相似度（✅）**：$\cos(u, v) = \dfrac{u \cdot v}{\|u\| \|v\|}$，只看方向不看长度。（数值丢失。）

**#9 二次插值（✅）**：给定 3 个点可唯一确定二次多项式，代值求解即可（拉格朗日或待定系数法）。（数值丢失。）

**#7 贝叶斯（✅）**：经典的两公司次品率题。设 A 供应比例 $P(A)$、次品率 $P(D|A)$，B 同理，抽到次品来自 B 的概率是

$$P(B|D) = \frac{P(D|B)\,P(B)}{P(D|A)\,P(A) + P(D|B)\,P(B)}$$

这道题我做对了，说明贝叶斯公式已经内化了。注意它是选择题里少有的"条件概率 + 全概率"综合题，属于必拿分。（原题数值丢失。）
...
为什么 SVD 能省空间？因为原始矩阵存的是 $mn$ 个元素，而低秩近似只存"分解出来的骨架"：左奇异向量、奇异值、右奇异向量，三者加起来远小于 $mn$（当 $k \ll \min(m,n)$）。这题的教训是：**公式会背 ≠ 考场算对**，SVD 存储公式要能当场推一遍而不是凭记忆。（原题数值丢失。）
...
$$R = r_A^{+} + r_B^{-} - \frac{r_C^{+} + r_D^{+}}{2}$$

其中 $r^{+}$ 是成功奖励、$r^{-}$ 是失败惩罚。关键陷阱是：**被跳过的 C、D 本身不获得任何奖励或惩罚**，只是它们的奖励平均值作为级联惩罚扣在 B 头上。（原题数值丢失。）
...
> 已知卷积网络梯度矩阵 $G$，秩 $r$（原题给了具体数，粘贴丢失）。说法：A. 零空间维度高 → 存在多个方向使梯度投影为零，一阶变化为零，易陷入平坦区/鞍点；B. 零空间维越高 → 有效信息维度越低，无效方向越多；C. 零空间维度 = 列数 − 秩；D. 增大 batch 可直接降低零空间维度（错）。答案 A、B、C。

核心是**秩—零空间定理**（Rank-Nullity Theorem）：对 $m \times n$ 矩阵，$\text{rank}(G) + \text{nullity}(G) = n$（列数）。零空间里的方向 $v$ 满足 $Gv = 0$，意味着沿 $v$ 更新参数时所有样本的梯度贡献都是零——损失函数沿这些方向的一阶变化为零，梯度下降在这些方向上看不到"下坡路"，所以容易卡在鞍点或平坦区。这就是"梯度冗余 → 优化困难"的机理。D 错在：batch 大小影响梯度估计的噪声和矩阵结构，但不能保证零空间维度降低。
...
> 用户查询向量 vs 历史故障案例向量，算余弦相似度保留 3 位小数，再判断匹配等级（极高/高/中等阈值）。答案：余弦相似度 ≈ 0.98（数值粘贴丢失），属于极高匹配。

这是"计算 + 阈值判断"两段式题。我会公式但算错或判断错了。教训：这种题要**一步步写**——内积、两个模、比值、对照阈值，中间任何一步粗心都会丢分。RAG 检索的核心就是 embedding 向量相似度，这是大模型应用的必考方向，需要练熟。
