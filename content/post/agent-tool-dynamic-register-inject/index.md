...
> 参考：[沐阳 — Skill 装太多，如何提升命中率？4 个实用方法讲透](https://x.com/yyyole/article/2061725917215605236)

上文从架构视角讨论了 Tools → Skills 的迁移可行性。而实际工程中，提升 Skills 命中率有四个经过验证的具体方法，它们和本文的工具注入架构形成了有趣的对照：

**方法 1：优化 Skill 描述**

不要只写"这个 Skill 能做什么"，要写清"什么时候该用、什么时候不该用"。一个合格的描述包含 5 个要素：解决的问题、适用场景、输入输出、不适用场景、与相似 Skill 的区别。

这与本文中工具 Schema 的 `description` 约束策略一脉相承——从 prompt 层约束调用意愿。

**方法 2：建立 Skill Tree（分层路由）**

当 Skill 超过 100 个时，平铺列表必然导致选择困难。正确做法是分层：先判断意图大类，再判断子类，最后选择具体 Skill。

```
用户任务 → 意图分类（信息查询 / 内容处理 / 执行动作）
         → 来源分类（公网 / 邮箱 / 文件）
         → 具体 Skill 选择
```

这和本文的"意图驱动注入"是同一种模式——先用分类缩小候选集，再做精确选择。

**方法 3：负样本描述**

写清楚"什么时候不用"比写"什么时候用"更重要。每个 Skill 至少写三类负样本：

- **相似但不该用的任务**（写邮件 vs 发邮件）
- **容易混淆的相邻 Skill**（公网搜索 vs 知识库搜索）
- **高风险操作的确认边界**（支付类必须先确认）

这实际上把"权限过滤"和"描述约束"结合为了一个可执行的规则。

**方法 4：召回 + 重排机制**

当 Skill 上百个时，不能全塞进 prompt 让模型选。应设计为检索系统：

```
用户 query → 召回候选（向量/关键词）→ 规则过滤 → 模型重排 → 执行
```

这与本文的 ToolOrchestrator + AgentInjector 架构完全同构。区别在于工具注入是系统层的 Schema 筛选，而 Skill 路由是 Agent 行为层的知识筛选。

**四个方法共同揭示了一个规律：当候选项超过模型的注意力边界时，不要指望模型自己解决，而应该用工程手段缩小候选范围。**
